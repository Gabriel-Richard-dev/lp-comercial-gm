 ...
 * define justamente para série de dado: matizes distantes entre si e contraste
 * de 3:1 contra o fundo, auditados em design/APLICACAO.md. Os getters leem o
 * CSS no momento do uso, então trocar o tema troca o mapa junto.
 */
const cat = (label, token) => ({
  label,
  token,
  get color() {
    return cssColorInt(token);
  },
  get hex() {
    return cssColorHex(token);
  },
});

export const CATEGORIES = {
  vestuario: cat("Vestuário", "--color-chart-1"),
  alimentacao: cat("Alimentação", "--color-chart-2"),
  eletronicos: cat("Eletrônicos", "--color-chart-3"),
  acessorios: cat("Acessórios", "--color-chart-4"),
  servicos: cat("Serviços", "--color-chart-5"),
};
